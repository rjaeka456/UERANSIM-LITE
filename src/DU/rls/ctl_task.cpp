...
void RlsControlTask::onLoop()
{
    ...
    case NtsMessageType::DU_RLS_TO_RLS: {
        ...
        case NmDURlsToRls::DOWNLINK_DATA:
            ...
            break;
        case NmDURlsToRls::DOWNLINK_RRC:
...
void RlsControlTask::handleRlsMessage(int ueId, int64_t sti, rls::RlsMessage &msg)
{
    ...
    else if (msg.msgType == rls::EMessageType::PDU_TRANSMISSION)
    {
        ...
        {
            auto w = std::make_unique<NmDURlsToRls>(NmDURlsToRls::UPLINK_DATA);
            w->ueId = ueId;
...
void RlsControlTask::handleDownlinkRrcDelivery(int ueId, uint32_t pduId, rrc::RrcChannel channel, OctetString &&data)
{
    ...
    if (pduId != 0)
    {
        if (m_pduMap.count(pduId))
        {
            ...
            auto w = std::make_unique<NmDURlsToRls>(NmDURlsToRls::RADIO_LINK_FAILURE);
...
void RlsControlTask::onAckControlTimerExpired()
{
    ...
    {
        auto w = std::make_unique<NmDURlsToRls>(NmDURlsToRls::TRANSMISSION_FAILURE);
